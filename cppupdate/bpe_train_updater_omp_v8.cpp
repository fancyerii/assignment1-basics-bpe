...
void bpe_train_step2(int vocab_size, 
    ...
    std::chrono::steady_clock::time_point max_start;
    size_t num_buckets;
    LocalMax max;
    ...
    #pragma omp parallel
    {
        while(true){
            ...
            #pragma omp master
            {
                loop_end = (vocabulary.size() == vocab_size);
                ...
                max_start = std::chrono::steady_clock::now();
                max.max_count = -1;
                
                num_buckets = pair_counts.bucket_count();
            }
            ...
            #pragma omp master
            {
                ...
                const auto& max_pair = max.max_pair;
...
int main(int argc, char* argv[]) {   
    ...
    std::cout << "CACHE_LINE: " << CACHE_LINE << std::endl;
