...
emhash8::HashMap<std::pair<int, int>, emhash8::HashSet<int>, pair_hash> read_pair_wordids(std::string dir){
    ...
    for (auto const& [key, val] : data.items()) {
        ...
        emhash8::HashSet<int> set;
        for(const auto & item : v){
            set.insert(item);
        }
        pair_wordids[pair] = set;
    }
    return pair_wordids;
}
...
void fine_grained_pair_counter_diff(const emhash8::HashSet<int>& affected_words,
                                    std::unordered_map<int, std::vector<int>> & wordid_encodings,
...
void updated_affected_word_count(const std::pair<int, int>& merge_pair,
    ...
    const emhash8::HashSet<int> affected_words_copy(affected_words);
...
void bpe_train_step2(int vocab_size, 
    ...
    auto heap_start = std::chrono::steady_clock::now();
    std::vector<HeapItem> pair_heap;
    for(const auto&[pair, count] : pair_counts){
        //heappush(pair_heap, {count, pair_strings[pair], pair});
        pair_heap.push_back({count, pair_strings[pair], pair});
    }
    heapify(pair_heap);
    auto heap_end = std::chrono::steady_clock::now();
    std::cout << "make heap: " << std::chrono::duration_cast<std::chrono::milliseconds>(heap_end - heap_start).count()
         << "ms" << std::endl;
...
int main(int argc, char* argv[]) {   
    if(argc != 4){
        std::cout << "usage: bpe_train_updater_fine_grained data_dir vocab_size out_dir" << std::endl;
        return -1;
    }
