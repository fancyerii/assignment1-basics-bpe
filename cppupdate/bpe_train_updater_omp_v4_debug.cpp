...
void bpe_train_step2(int vocab_size, 
    ...
    while(vocabulary.size() < vocab_size){
        if(vocabulary.size() % 1000 == 0){
            ...
            std::cout << "    pair_counts.bucket_count(): " << pair_counts.bucket_count() << std::endl;
        }
        ...
        if(vocabulary.size() % 100 == 0){
            std::cout << "###compare " << vocabulary.size() << " pair_counts: " << pair_counts.size() << std::endl;
            auto max_ms = std::chrono::duration_cast<std::chrono::milliseconds>(max_end - max_start);
            std::cout << "non parallel: " << max_ms.count() << " ms." << std::endl;
            auto start = std::chrono::steady_clock::now();
            int max_count = -1;
            std::pair<int, int> max_pair;
            std::vector<std::vector<int>> max_strings;            
            #pragma omp parallel
            {
                ...
                auto thread_ms = std::chrono::duration_cast<std::chrono::milliseconds>(thread_end - thread_start);
                std::cout << "for loop of thread " << thread_id <<": " << thread_ms.count() << " ms. visited: " << visited << std::endl;                
                ...
                auto lock_ms = std::chrono::duration_cast<std::chrono::milliseconds>(lock_end - lock_start);
                std::cout << "lock of thread " << thread_id <<": " << lock_ms.count() << " ms." << std::endl;                              
            }
            auto end = std::chrono::steady_clock::now();
            auto max_omp_ms = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
            std::cout << "omp: " << max_omp_ms.count() << " ms." << std::endl;
...
int main(int argc, char* argv[]) {   
    if(argc != 3){
        std::cout << "usage: bpe_train_updater data_dir vocab_size" << std::endl;
        return -1;
    }
    int vocab_size = std::stoi(argv[2]); 
    std::string data_dir(argv[1]);

    std::cout << "vocab_size: " << vocab_size << ", data_dir: " << data_dir << std::endl;
    ...
    
    {
        ...
        std::ofstream output_file(data_dir + "/vocabulary_new.json");
...

    {
        ...
        std::ofstream output_file(data_dir + "/merges.json");
        output_file << json_string;
        output_file.close();
    }
