...
class BPE_Trainer():
    def train(self, input_path, vocab_size, special_tokens, *args):
        ...
        for pair, count in pair_counts.items():
            maxheap.heappush(pair_heap2, (count, pair_strings[pair], pair))
        end_time = time.perf_counter()
        print(f"heappush: {end_time - start_time:.2f}s")
        ...
        end_time = time.perf_counter()
        print(f"heapify: {end_time - start_time:.2f}s")
        ...
        end_time = time.perf_counter()
        print(f"merge: {end_time - start_time:.2f}s")
