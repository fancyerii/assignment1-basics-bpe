...
class BPE_Trainer():
    def train(self, input_path, vocab_size, special_tokens, *args):
        ...
        num_max_processes = args.num_max
        ...
        with mp.Pool(processes=num_max_processes) as pool:
            ...
            while size < vocab_size:
                ...
                size += 1
                if size % args.print_interval == 0:
                    print(f"size: {size} original time {times[0]}, copy pair: {times[1]}"
                          f", copy pairstring: {times[2]}, parallel max: {times[3]}"
                          f", max counter: {times[4]}")
    ...
    def _find_max_pair(pair_strings, pair_counts):
        ...
        return max_count, pair_strings[max_pair], max_pair

    @staticmethod
    def _parallel_max(pair_counts, pair_strings, pool, num_processes):
        ...
        # merge(max) final results is quick, so we don't use imap_unordered
    ...
    def _merge_a_pair(pair_counts, pair_strings, vocabulary, pair_to_words, 
        ...
        # start_time = time.perf_counter()
        # max_count2, pair_str, merge_pair2 = BPE_Trainer._parallel_max(pair_counts_list, pair_strings, pool, num_processes)
        # end_time = time.perf_counter()
        # times[3] += (end_time - start_time)   
        # assert merge_pair == merge_pair2
        # assert max_count == max_count2
        # max_count, _, merge_pair = BPE_Trainer._parallel_max(pair_counts, pair_strings, pool, num_processes)
