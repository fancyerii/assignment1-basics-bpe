...
class BPE_Trainer():
        ...
        with mp.Pool(processes=num_max_processes) as pool:
            ...
            while size < vocab_size:
                BPE_Trainer._merge_a_pair(pair_counts, pair_strings, vocabulary,
                                    ...
                                    merges, size, pool, num_max_processes, max_time, update_time)
                size += 1
    ...
    def _find_max_pair(pair_strings, pair_counts):
        ...
        return max_count, pair_strings[max_pair], max_pair, time_used
    ...
    def _parallel_max(pair_counts, pair_strings, pool, num_processes):
        ...
        end_time = time.perf_counter()
        print(f"copy time: {end_time - start_time}")
        ...
        end_time = time.perf_counter()
        print(f"pool.map time: {end_time - start_time}")
        computing_time = sum([r[-1] for r in local_maxes])
        print(f"computing_time: {computing_time}")
    ...
    def _merge_a_pair(pair_counts, pair_strings, vocabulary, pair_to_words, 
        ...
        # max_count2, pair_str, merge_pair2 = BPE_Trainer._parallel_max(pair_counts, pair_strings, pool, num_processes)
        # assert merge_pair == merge_pair2
        # assert max_count == max_count2
        start_time = time.perf_counter()
        _, _, merge_pair = BPE_Trainer._parallel_max(pair_counts, pair_strings, pool, num_processes)
        end_time = time.perf_counter()
