...
import os
import gc
from cs336_basics.bpe_train_step2_wrapper import py_bpe_train_step2_list
...
class BPE_Trainer():
    def train(self, input_path, vocab_size, special_tokens, *args):
        ...
        input_list = [vocab_size, pair_counts, pair_strings, vocabulary,
                      pair_to_wordids, wordid_counts, wordid_encodings, merges]
        ...
        pair_strings = None
        vocabulary = None
        pair_to_wordids = None
        wordid_counts = None
        wordid_encodings = None
        merges = None
        ...
        print("before py_bpe_train_step2_list")
        ...
        start_time = time.time()
        merges_cpp, vocabulary_cpp = py_bpe_train_step2_list(input_list)
        end_time = time.time()
        print(f"py_bpe_train_step2: {end_time - start_time:.2f}")

        mem_info = current_process.memory_info()
        print("after py_bpe_train_step2_list")
        print(f"\tCurrent Memory Usage (RSS): {mem_info.rss / 1024 / 1024:.2f} MB")
        print(f"\tCurrent Memory Usage (VMS): {mem_info.vms / 1024 / 1024:.2f} MB")
